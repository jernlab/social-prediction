...
t_values = {
    'cake': cake_t_values,
    'movie': movie_t_values,
    'podcast': podcast_t_values
}
...
for story in stories:
    ...
    for level in range(1,6):
        ...
        for row in range(dfFilter.shape[0]):
            ...
            tidyDf = tidyDf.append({'Story': story, 'Level': level, 'Context': 1, 'Rating': context1, 'Subject': subject, 'TVal':t_values[story][level-1]}, ignore_index=True)
            tidyDf = tidyDf.append({'Story': story, 'Level': level, 'Context': 2, 'Rating': context2, 'Subject': subject, 'TVal':t_values[story][level-1]}, ignore_index=True)
