...
gridSearch <- function(x,y,b0s, b1s){
  ...
  y_lv <- as.data.frame(sapply(x, function(t){
    return((y %>% filter(T_val == t) %>% select(Prediction))$Prediction)
  }))
  ...
  for(b1 in b1s){
    ...
    for(b0 in b0s){
      ...
      pr1 <- MSE(y1,scPredRep[,1])
      pr2 <- MSE(y2,scPredRep[,2])
      pr3 <- MSE(y3,scPredRep[,3])
      pr4 <- MSE(y4,scPredRep[,4])
      pr5 <- MSE(y5,scPredRep[,5])
      # print(scPred)
...
Testing predictions
```{r}
newModel <- createSocialModel(-4.5,35)
