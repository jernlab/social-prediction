...
if(socialOrNonSocial == SOCIAL):
	df = pd.read_csv("FA21_22_SOCIAL.csv")
	print("Tidying social results")
else:
	df = pd.read_csv("FA21_22_NON_SOCIAL.csv")
...
for story in stories:
    ...
    for row in range(dfFilter.shape[0]):	
	    for i in range(5):
	    	...
	    	tidyDf = tidyDf.append({'Story': story, 'Level': level, 'Context': "Social" if socialOrNonSocial == SOCIAL else "Non-Social", 'Prediction': prediction, 'Subject': row+1}, ignore_index=True)


if(socialOrNonSocial == SOCIAL):
	tidyDf.to_csv('social-tidy-pilot-12-30-21.csv', index=False)
else:
	tidyDf.to_csv('nonsocial-tidy-pilot-12-30-21.csv', index=False)
